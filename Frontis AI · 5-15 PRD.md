...
| 成果文件预览   | 预览面板           | 定义 ME 输出文件在文件卡片、今日成果、成果列表中的预览类型   | 4.2.8    |
| 工作记录       | 工作记录           | 仅 ME 页展示入口，记录按日期倒序，可搜索和筛选               | 4.2.9    |
...
ME 可以创建或编辑常见办公、代码、网页和媒体文件。所有由 ME 创建或编辑的文件都进入同一套成果文件模型，并在任务结束文件卡片、右上角今日成果、成果列表和工作记录中复用。文件卡片只展示摘要；用户点击文件后进入预览面板，预览面板按文件类型展示对应查看器。

| 类型         | 文件扩展名 / MIME 示例                              | 成果列表展示             | 预览方式                                                                 | 交互规则                                                       |
| ------------ | ---------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------ | -------------------------------------------------------------- |
| 文档 / 富文本 | `md`、`markdown`、`txt`、`doc`、`docx`、`rtf`、`odt` | 文档图标、文件名、大小、产出时间 | 富文本阅读视图；支持标题、段落、列表、表格、代码块和图片混排             | 点击文件卡片打开文档预览；文档内图片按容器宽度等比展示         |
| 表格         | `csv`、`tsv`、`xls`、`xlsx`、`ods`                  | 表格图标、文件名、大小、产出时间 | 工作簿预览；展示名称框、公式栏、列字母、行号、单元格网格和 Sheet 页签     | 支持切换 Sheet；超出区域横向和纵向滚动；默认选中第一个单元格    |
| 演示文稿     | `ppt`、`pptx`、`odp`                                | 演示图标、文件名、大小、产出时间 | 幻灯片预览；主画布展示当前页，底部展示所有幻灯片缩略图                   | 点击缩略图切换当前页；每页展示标题、说明、要点和配图           |
| HTML 页面    | `html`、`htm`、`text/html`                          | HTML 图标、文件名、大小、产出时间 | 页面预览和源码预览双模式                                                 | 默认展示页面渲染效果；支持切换源码；支持新窗口打开页面预览     |
| 代码文件     | `json`、`py`、`js`、`ts`、`tsx`、`sql`、`yaml`、`xml`、`css`、`less`、`sh` 等 | 代码图标、文件名、大小、产出时间 | 代码阅读器；展示语言标签、行号和基础语法高亮                             | 保留缩进和换行；支持横向滚动；不把代码按普通文本折行           |
| 图片         | `png`、`jpg`、`jpeg`、`gif`、`webp`、`svg`、`avif`   | 图片图标、文件名、大小、产出时间 | 图片预览                                                                 | 图片按预览区域等比适配，不拉伸变形                             |
| 视频         | `mp4`、`webm`、`mov`、`m4v`                         | 视频图标、文件名、大小、产出时间 | 视频播放器；可展示封面海报、播放区域和文件信息                           | 使用浏览器原生播放控件；无法播放时保留下载入口                 |
| 音频         | `mp3`、`wav`、`m4a`、`aac`、`flac`                  | 音频图标、文件名、大小、产出时间 | 音频播放器；展示音频封面、文件信息、波形视觉和播放控件                   | 使用浏览器原生播放控件；无法播放时保留下载入口                 |
| PDF          | `pdf`、`application/pdf`                            | PDF 图标、文件名、大小、产出时间 | PDF 内嵌预览                                                             | 使用浏览器 PDF 预览能力；不支持时保留下载入口                  |
| 其他文件     | 未命中以上类型                                      | 通用文件图标、文件名、大小、产出时间 | 不提供内嵌预览                                                           | 展示“不支持预览该类型文件”，保留下载入口                       |

成果文件预览规则：

1. 预览类型优先按 MIME 判断；MIME 缺失或不可信时按文件扩展名判断。
2. 同一个成果文件在文件卡片、今日成果、成果列表和工作记录中使用同一份文件元数据，不重复生成不同卡片。
3. 同一路径文件被多次创建或编辑时，成果列表和今日成果只展示最新版本；工作记录可保留历史过程摘要。
4. 文件被删除或失效时，文件卡片保留文件名和状态，预览面板展示“文件已删除或无法预览”，不展示旧内容。
5. 预览面板始终保留下载入口；预览失败不影响用户下载原文件。
