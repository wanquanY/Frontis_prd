...
| 平台公开    | AI 专家或 Skill 面向平台范围公开，不只在当前企业内可见                                 |
| 企业内发布  | AI 专家或 Skill 仅在当前企业租户内可见可用                                             |
...
8. 创建租户、邀请成员、编辑成员时，只能选择角色管理中已创建且可分配的角色。

### 2.3 租户创建原则

1. 运营创建租户时不选择计费模式。
2. 运营创建租户时不选择“是否开通运营系统”。
3. 运营创建租户时必须录入初始管理员，并为初始管理员选择角色。
4. 初始管理员能进入哪些系统，由所选角色的权限决定。
...
6. 没有直接平台公开权限时，AI 专家仍可先发布到企业内，再提交平台公开申请，由运营在 AI 专家上架审批中审核。
7. 运营审核通过 AI 专家平台公开申请后，系统在商品中心生成一条待完善商品，运营需补齐商品信息后才能上架。
...
| 工作台       | ME              | 使用 ME                                            |
| 工作台       | 专家列表        | 使用专家列表                                       |
| 工作台       | 进化实验室      | 开发 AI 专家与 Skill                               |
| 工作台       | 进化实验室      | 发布 AI 专家到专家广场（企业内）                   |
| 工作台       | 进化实验室      | 提交 AI 专家进入专家广场平台公开申请               |
| 工作台       | 进化实验室      | 直接发布 AI 专家到专家广场（平台公开）             |
| 工作台       | 进化实验室      | 直接发布 Skill 到技能中心（平台公开）              |
| 管理后台     | AI 专家管理     | AI 专家管理                                        |
| 管理后台     | 组织管理        | 组织管理                                           |
| 管理后台     | 角色管理        | 角色管理                                           |
| 运营管理平台 | 租户管理        | 租户管理                                           |
| 运营管理平台 | 组织管理        | 组织管理；复用管理后台同一套组织树、成员和角色模型 |
| 运营管理平台 | 角色管理        | 角色管理；复用管理后台同一套角色和权限项           |
| 运营管理平台 | 商品中心        | 商品中心                                           |
| 运营管理平台 | AI 专家上架审批 | AI 专家上架审批                                    |
| 运营管理平台 | 运营配置        | 用户交流群配置；自动注册配置和新用户初始化模板配置 |

说明：管理后台和运营管理平台中的组织管理、角色管理复用同一套组织与角色数据，不创建两套组织或两套角色。页面入口可分别出现在两个系统菜单下，但底层成员、部门、角色、权限项保持一致。
...
| 租户创建     | 创建租户时必须为初始管理员选择一个角色管理中已创建且可分配的角色                     |
| 成员管理     | 邀请成员和编辑成员时，只能选择当前租户角色管理中已创建且可分配的角色                 |
| 无可选角色   | 当前没有可分配角色时，不能完成租户创建、成员邀请或成员角色变更，需要先创建角色       |
...
| 统一工作卡片   | 进度/今日成果      | 进度展示本轮专家任务；今日成果汇总当日 ME 产出文件           | 4.2.6    |
...
统一工作卡片只在 ME 对话页展示。其他 AI 专家独立对话页不展示工作记录入口，也不展示 ME 的任务进度卡片。

| 区域     | 触发条件                         | 展示字段                                             | 展开状态                                                             | 收起状态                                               |
| -------- | -------------------------------- | ---------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------ |
| 进度     | 本轮任务调用任意 AI 专家          | 任务名称、AI 专家头像、AI 专家名称、任务状态          | 展示本轮全部 AI 专家任务，按调用顺序排列                             | 只展示当前最新进度和状态                               |
| 今日成果 | 当日存在 ME 创建或编辑的产出文件  | 文件图标、文件名、文件类型、最近更新时间              | 展示当日全部产出文件，按最近更新时间倒序排列                         | 可保留入口或展示最新文件摘要，点击后进入展开态         |

进度状态只允许三种：执行中、执行完成、执行失败。未调用 AI 专家时不展示进度区；没有当日文件产出时不展示今日成果空列表。
...
4. 第二行展示分类 Tab：全部 + 运营商品中心启用的技能中心分类。
5. 不展示“公共 / 我的”切换。
6. 不展示 Workflow、Skill、模型等类型筛选。
...
列表展示租户名称、租户编码、租户类型、创建来源、版本、初始管理员、初始角色、席位数量、模块范围、租户状态、更新时间和操作。租户类型包括个人型租户和团队型租户；创建来源包括自动注册和运营创建。
...
| 初始管理员角色   | 是   | 从角色管理中已创建且可分配的角色中选择，决定初始管理员可进入哪些系统和可使用哪些功能 |
| 席位数量         | 是   | 必须大于等于 1；席位数为 1 推导个人版，大于 1 推导团队版 |
| 生效时间         | 是   | 租户合同或服务开始日期                                   |
| 失效时间         | 是   | 租户合同或服务结束日期                                   |

创建租户时不出现计费模式字段，不出现运营系统开通字段。

#### 6.2.3 编辑租户

编辑租户支持修改租户名称、租户编码、行业、初始管理员姓名、初始管理员手机号、初始管理员角色、席位数量、生效时间、失效时间。初始管理员角色只能改为角色管理中已创建且可分配的角色；席位数量不能小于当前已加入成员数量。
...
| 租户创建 -> 初始管理员         | 运营创建租户并选择角色   | 系统创建初始管理员成员，成员获得所选角色权限       |
...
7. ME 右上角统一工作卡片只在调用 AI 专家时展示进度区；展开态展示本轮全部专家任务，收起态只展示最新进度和状态；今日成果汇总当天由 ME 创建或编辑的文件。
...
11. 技能中心没有外层页面标题；顶部只保留 Skill / MCP 切换、搜索和分类 Tab，不出现公共 / 我的和类型筛选。
12. 管理后台只展示 AI 专家管理、组织管理、角色管理；不展示模型配置、ME 管理、积分管理、驾驶舱。
13. 管理后台组织管理和角色管理是侧边栏独立页面，不是同一页面 Tab。
14. 运营管理平台只展示租户管理、组织管理、角色管理、商品中心、AI 专家上架审批、运营配置。
15. 运营创建租户弹窗不包含计费模式和运营系统开通字段，必须选择初始管理员角色。
16. 角色权限树按工作台、管理后台、运营管理平台分组；管理后台和运营平台权限到二级菜单即可，进化实验室保留开发 AI 专家与 Skill、发布 AI 专家到专家广场、提交平台公开申请、直接平台公开、发布 Skill 到技能中心等功能项。
17. 商品中心分类管理同时支持专家广场分类和技能中心分类。
18. AI 专家审批通过后只生成待完善商品，不直接在用户侧上架。
19. 登录页提供验证码登录和密码登录两个 Tab；切换 Tab 时保留手机号输入，验证码和密码字段互不串值。
20. 登录和注册共用手机号验证码准入；手机号已存在时登录，手机号不存在且自动注册开启时自动注册，关闭时提示“暂无权限”。
21. 自动注册账号创建完成后必须先设置登录密码；密码需要输入两次，两次一致且满足规则后才能继续。
22. 沈一新这类未设置密码的新用户账号，验证码校验通过后先弹出设置密码弹窗；密码设置成功后再进入 ME 首登资料弹窗。
23. 密码登录只允许已设置密码账号使用；未设置密码账号在密码登录 Tab 提示先使用验证码登录并设置密码。
24. 登录后多租户账号先选择租户；单租户账号直接使用该租户。
25. 选定租户后，单系统权限直接进入对应系统，多系统权限默认进入对话工作台。
26. 用户进入系统后可切换有权限的租户和系统；切换租户后重新计算可进入系统和默认落点。
27. 运营配置中可开关自动注册，并可增删新用户初始化能力；保存时至少保留一个能力。
28. 自动注册用户不绑定“普通成员”或“新用户”等固定角色，初始权限来自新用户初始化模板。
29. 自动注册用户升级或被重新分配角色后，权限按套餐和租户内已创建角色重新计算，不继续受初始化模板限制。
30. 系统初始化一个平台型组织和最高平台型租户；最高平台型租户拥有平台级运营权限，可进入运营管理平台使用租户管理、商品中心、AI 专家上架审批、运营配置、平台组织管理和平台角色管理。
31. 自动注册产生的个人型租户和运营创建的团队型租户，都必须出现在最高平台型租户运营管理平台的租户管理列表中。
