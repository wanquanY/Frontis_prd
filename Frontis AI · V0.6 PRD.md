...
| 消耗 | MetaAgent 调用 | 用户通过 MetaAgent 调用模型或调度专家    | 使用人     |
...
| 群聊会话 | 飞书机器人被拉入群聊后按群创建 | 管理员可配置群聊维度的 `user.md`、`soul.md` 和可调度 AI 专家列表 | 由管理员从当前租户可见且可用的 AI 专家商品中添加和移除，作为该群聊内 MetaAgent 的可调度范围 |
...
| 前后端 | 积分管理 | 积分流水 | 公有云租户默认展示积分流水，支持本周、本月、全部时间筛选，以及全部、增加、消耗类型筛选；字段包括类型、来源、积分、操作人、时间；来源包括注册送积分、邀请奖励、购买标准积分包、MetaAgent 调用、具体 AI 专家名称、Skill 名称、第三方接口名称；增加类操作人统一显示 FrontisAI，消耗类操作人显示使用人。 | 功能 | V0.6 | P0 | 杨万权 |
...
| 前后端 | MetaAgent 管理 | 会话配置 | 点击会话的进入配置后进入二级页面，按平台内绑定成员会话、外部会话、群聊会话区分配置逻辑；平台内绑定成员会话与用户工作台 MetaAgent 绑定，可查看 user.md、soul.md 和权限范围内可调度专家，但不在会话内增删专家；外部会话和群聊会话不绑定平台成员，由管理员维护 user.md、soul.md，并从当前租户可见且可用的 AI 专家商品中添加或移除可调度专家。 | 功能 | V0.6 | P0 | 杨万权 |
