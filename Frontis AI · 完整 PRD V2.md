...
- 员工工作台：MetaAgent、专家工作室、AI专家广场、开发与进化
...
1. 员工进入工作台后，默认直接进入 `MetaAegnt` 菜单。
2. `MetaAegnt` 采用单线程持续对话模式，不区分多会话，不展示 `新对话` 按钮和历史会话列表，所有对话记录都持续沉淀在同一条线程中。
3. `MetaAegnt` 会在当前用户已有权限的 AI 专家范围内自动判断是否需要调度其它专家协同处理。
4. 员工可进入 `专家工作室` 查看并使用其它已添加到工作台的 AI 专家；这些专家仍保留独立会话与历史会话管理。
5. 员工在 AI专家广场中浏览当前租户可用的 AI 专家，包括企业自研专家与 FrontisAI发布的专家。
6. 员工可将当前有权限使用的 AI 专家直接添加到 `专家工作室`。
7. 员工在开发与进化中开发自己的 AI 专家与 Skill。
8. 员工可将自己开发并已发布的 AI 专家直接添加到 `专家工作室` 使用。
9. 员工可发布 AI 专家为 `个人 / 团队共享 / 企业公开`，也可按对应规则发布 Skill。
10. 如果当前租户已开通 AI 专家开发服务，员工可在自己开发的 AI 专家详情中申请上架。

```mermaid
flowchart TD
  A["进入工作台"] --> B["默认进入 MetaAegnt"]
  B --> C["进入 AI专家广场"]
  C --> D["添加当前可用专家到专家工作室"]
  B --> E["切换到专家工作室发起多 topic 会话"]
  B --> F["进入开发与进化"]
  F --> G["开发 AI 专家 / Skill"]
...
|---|---|---|---|---|
| MetaAgent | 入口定位 | 默认进入 | 员工进入工作台后默认进入 `MetaAegnt` 菜单。`MetaAegnt` 作为系统默认协同入口，负责承接用户首次进入系统后的默认任务发起。 | P0 |
| MetaAgent | 会话管理 | 单线程会话 | `MetaAegnt` 不提供 `新对话` 按钮，也不展示历史会话列表。全部消息、调度过程和后续追问都持续沉淀在同一条工作线程中。 | P0 |
| MetaAgent | 会话交互 | 发送消息 | 用户可通过自然语言输入向 `MetaAegnt` 发起任务请求，围绕同一条线程持续补充信息并推进任务。 | P0 |
| MetaAgent | 会话交互 | 协同调度 | `MetaAegnt` 在收到任务后，应先理解用户意图，再从“当前用户已拥有权限的 AI 专家集合”中判断是否需要调度其他专家协同处理。被调度的专家应在会话流中以独立 AI 专家身份出现，最后再由 `MetaAegnt` 做统一汇总与交付。 | P0 |
| MetaAgent | 会话交互 | 权限内专家调度范围 | `MetaAegnt` 的调度范围只能是当前用户已经有权限使用的专家集合。用户看不到、用不到的专家，也不能被 `MetaAegnt` 在后台调起参与当前任务。 | P0 |
| MetaAgent | 会话交互 | 快捷问题 | 为降低首次使用门槛，MetaAgent 页面可提供预置快捷问题或推荐提问模板，帮助用户直接开始协同任务。 | P1 |
| MetaAgent | 会话交互 | 附件上传 | 用户在发送消息前或会话过程中可上传文档、表格、图片等附件，让 `MetaAegnt` 在首轮或后续轮次获取完整业务材料。 | P0 |
| MetaAgent | 会话交互 | 停止生成 | 当当前轮输出方向偏离预期、生成内容已经足够、或用户希望立即中止本轮时，可主动停止生成。 | P1 |
| MetaAgent | 成果管理 | 成果列表 | `成果` 指当前会话过程中产生的全部文件型输出，包括文档、表格、图片、Markdown、导出文件及其他可落地保存的文件。 | P0 |
| MetaAgent | 成果管理 | 成果预览与继续使用 | 用户可对会话中产生的成果进行预览、再次打开、继续补充使用或带入后续会话。 | P1 |
| MetaAgent | 结果展示 | 结构化结果卡片 | 对于摘要、结论、步骤建议、提炼要点等非文件型输出，系统以结构化结果卡片方式展示。 | P1 |
| 专家工作室 | 专家列表 | 已添加专家列表 | `专家工作室` 统一展示当前账号已经添加到工作台并可直接使用的其他 AI 专家，包括企业自研专家、FrontisAI发布专家和用户自己主动添加的专家。 | P0 |
| 专家工作室 | 会话管理 | 新建会话 | 用户可围绕某一个具体 AI 专家发起新的独立会话，新会话默认继承当前专家身份，但不继承其他历史会话的上下文。 | P0 |
| 专家工作室 | 会话管理 | 历史会话 | 用户可在左侧历史会话区回看同一专家下的历史对话，并支持按时间切换和继续追问。 | P0 |
| 专家工作室 | 会话交互 | 一对一使用 | 当用户在 `专家工作室` 中主动选择某位 AI 专家时，该专家应进入一对一单独使用模式。当前会话只围绕该专家自身能力展开，不再由 `MetaAegnt` 作为调度入口介入，也不应自动拉起其它专家加入。 | P0 |
...
1. 用户工作台默认菜单为 `MetaAegnt`，它始终作为系统默认协同入口存在。
2. `专家工作室` 中的其他 AI 专家来源于当前账号在企业内可用的企业自研专家与 FrontisAI发布专家，也包括用户自己开发并可见的个人专家。
3. `个人` AI 专家仅开发者自己可见。
...
1. 员工工作台当前拆分为两个入口：`MetaAegnt` 和 `专家工作室`。
2. 用户进入工作台后默认落在 `MetaAegnt`，系统进入 `MetaAegnt 单线程协同模式`：
...
5. 当用户切换到 `专家工作室` 并主动选择某个其它 AI 专家时，系统进入 `单专家一对一模式`：
   - 当前会话只绑定当前专家；
   - 专家直接围绕自己的能力处理任务；
   - 不再自动调度其它专家加入；
   - 其他 AI 专家仍支持新建独立会话与历史会话切换。
6. `MetaAegnt` 的定位是“默认协同入口”；`专家工作室` 的定位是“其它专家的一对一使用入口”，两者在菜单层即应明确分离。
