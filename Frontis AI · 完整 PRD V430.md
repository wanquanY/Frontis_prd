...
17. 用户可通过 `工作轨迹` 搜索、筛选和按时间轴浏览历史工作记录，并定位到原始对话位置。
18. 用户可进入 `专家工作室` 使用其他 AI 专家的多 topic / 多会话能力。
19. 团队版管理后台在 `组织管理` 下提供二级菜单 `角色管理`，用于查看平台预设角色、创建自定义角色并配置可下放权限。
20. 运营平台提供 `组织管理`，支持创建运营账号、维护运营角色，以及创建和编辑租户侧全平台预设角色。
...
1. 个人版管理员进入 `管理后台`。
2. 在 `租户总览` 看到当前版本为 `个人版`，不展示组织管理和成员邀请。
3. 管理员看到当前积分余额、近期消耗和购买积分入口。
4. 管理员进入 `积分管理`，查看本人维度的积分消耗统计。
...
2. 团队版开通后，管理后台展示组织管理、成员邀请、席位和多人消耗统计。
...
| 团队版本 | 版本状态 | 个人版/团队版能力差异 | 管理后台展示当前版本，并按版本控制组织管理、成员邀请和多人消耗统计。 | P0 |
...
4. 通过版本能力差异避免个人版看到不应出现的组织治理能力。

### 4.2 用户旅程

#### 4.2.1 个人版管理员查看租户状态与积分

1. 管理员进入 `租户总览`。
2. 看到当前版本为个人版。
3. 页面展示当前积分余额、近期消耗、本人使用概况。
4. 页面不展示组织管理和成员邀请入口。
5. 管理员可进入 `积分管理` 查看消耗统计或购买积分。
6. 团队版升级入口可作为弱提示或后续能力保留，不作为个人版一期主操作。
...
| 功能模块 | 一级功能 | 二级功能 | 功能描述 | 优先级 |
|---|---|---|---|---|
| 租户总览 | 版本管理 | 个人版状态 | 展示当前租户是否为个人版，并明确当前为单人使用，不展示组织管理入口。 | P0 |
| 租户总览 | 版本管理 | 团队版状态 | 展示当前团队套餐、包含席位数和有效期。 | P0 |
| 租户总览 | 版本购买 | 开通团队版 | 作为底层能力和后续转化入口保留，个人版一期可弱化或隐藏。 | P1 |
...
9. 团队版才展示角色管理入口；个人版不展示组织管理和角色管理。
...
| 页面 | 页面目标 | 核心内容 |
|---|---|---|
| 租户总览 | 让管理员快速理解当前租户状态 | 当前版本、积分余额、近期消耗；个人版不展示组织管理 |
