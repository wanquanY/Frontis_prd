...
为便于按系统分别评审、排期和交付，当前正式 PRD 已拆分为以下 3 份独立文档：

1. 《Frontis AI · 工作台 PRD》
2. 《Frontis AI · 企业管理后台 PRD》
3. 《Frontis AI · FDE配置交付 PRD》

当前文档继续保留为总览版，适合做跨系统关系说明；详细评审优先使用拆分后的子 PRD。
...
| 企业管理员 | 用户工作台、企业管理后台 | 管理企业 AI 能力资产并参与使用 | 配置 AI 专家、设备、模型；也可使用工作台 | 企业管理后台 / 用户工作台 |
| 部门负责人 | 用户工作台 | 作为业务角色使用已授权 AI 专家 | 当前与普通员工无功能差异 | 用户工作台 |
| FDE 团队负责人 | FDE 业务管理 | 统筹商机、交付、资产和版本运营 | 查看全量业务数据、分配商机、管理成员、统筹交付与版本任务 | FDE 业务管理 |
| FDE 团队管理员 | FDE 业务管理 | 负责执行与管理业务运转 | 查看全量业务数据、分配商机、推进交付、查看资产、处理版本任务 | FDE 业务管理 |
| FDE 小组负责人 | FDE 业务管理 | 负责本小组业务推进与组内派发 | 查看本组商机、租户、资产与版本任务；可将本组商机分配给组员 | FDE 业务管理 |
| FDE 普通成员 | FDE 业务管理 | 跟进自己负责的业务任务 | 查看分配给自己的商机、订单、交付、资产与版本任务 | FDE 业务管理 |
...
3. **FDE 团队负责人 / 团队管理员 / 小组负责人 / 普通成员**
   - 属于交付侧角色，不进入企业管理后台
   - 通过 FDE 业务管理系统处理商机、租户、交付、资产和版本
   - 团队负责人和团队管理员看全量，小组负责人看本组，普通成员只处理自己的业务对象
...
### 4.2.2 企业管理员 / 部门负责人在工作台中的旅程

1. 企业管理员和部门负责人也可使用工作台内的 AI 专家。
...
| 对话主界面 | 输入框交互 | Skill 快选 | 用户可在发送前选择当前 AI 专家提供的 Skill，明确告诉 AI 本轮任务希望优先调动哪类能力，从而提升首轮回复的针对性和任务命中率。 | P1 |
...
2. 为专家完成设备绑定、组织范围权限配置、模型选择和版本处理。
...
| 功能模块 | 一级功能 | 二级功能 | 功能描述 | 优先级 |
|---|---|---|---|---|
| 驾驶舱 | 总览 | 经营总览卡片 | 为企业管理员提供一个“一眼看全局”的入口，集中展示任务量、运行时长、节省工时、活跃 AI 专家等核心指标，帮助管理层快速判断系统当前是否在持续产生业务价值。 | P1 |
...
| 我的AI专家团 | 配置流程 | 直接配置可用权限 | 对无需先绑定设备的 AI 专家，管理员可直接按组织范围设置开放对象，快速把专家能力开放给全公司、某些部门或指定个人，提高配置效率。 | P0 |
| 我的AI专家团 | 配置流程 | 按设备分配权限 | 当同一个 AI 专家被绑定到多台设备时，管理员可以分别按设备配置组织范围，确保不同设备上的专家能力开放范围与实际使用场景保持一致。 | P0 |
| 我的AI专家团 | 配置流程 | 组织树选择范围 | 权限配置基于组织树进行，支持企业、部门、个人三类授权主体，并支持多个并列部门或部门与个人混合授权。 | P0 |
...
2. 只有企业管理员可以进入企业管理后台。
3. 部门负责人和普通员工当前在功能上不做区分，统一不进入企业管理后台。
...
7. 设备所属员工通过组织树选择，但只能选择具体个人，不能直接选择部门。
...
| 待分配权限 | 当前专家尚未完成权限开放 | 未设置全公司或组织范围权限 |
...
### 6.2.1 FDE 团队负责人 / 团队管理员旅程

1. 进入首页看板，掌握销售金额、配置中订单、已交付租户和团队概括。
2. 进入商机管理，将商机分配给小组或成员，判断成单与否。
3. 进入租户管理，统筹全量租户、租户下订单与交付进度。
4. 进入客户资产管理，查看交付后资产、续费和变更。
5. 进入 AI 专家版本管理，处理租户下 AI 专家的版本任务。

### 6.2.2 FDE 小组负责人旅程

1. 可进入首页看板查看本小组业务总览。
2. 查看分配给本组的商机、租户、资产和版本任务。
3. 将已经归属到本组的商机继续分配给组员。
4. 推进本组范围内的订单交付、资产续费和版本运营。
...
| 首页看板 | 团队概括 | 成员概括总览 | 从成员视角展示配置交付数、运行租户数、版本任务数和已成交金额，帮助负责人综合判断每位成员的承接情况与业务产出。 | P2 |
| 商机管理 | 商机列表 | 商机列表展示 | 以统一列表承接当前所有商机线索，帮助 FDE 团队快速判断这条商机是系统自动同步还是人工补录、由谁创建、由谁负责、当前推进到什么阶段，避免商机分散在个人记录中。 | P1 |
| 商机管理 | 商机列表 | 手动创建商机 | 支持负责人或管理员把线下沟通、销售转交或会议纪要中的有效线索手动录入系统。手动创建时由系统自动记录创建方式、创建人和创建时间，录入人只需要补齐客户需求联系人与需求内容，避免人为伪造来源信息。 | P1 |
| 商机管理 | 商机详情弹窗 | 分配对象 | 团队负责人和团队管理员可将商机明确分配给小组或具体 FDE 成员；小组负责人可把归属本组的商机继续分配给本组成员，形成清晰的跟进责任归属。 | P1 |
| 商机管理 | 商机详情弹窗 | 修改商机状态 | 通过标准化状态记录商机当前进展，让团队能够快速识别哪些商机刚开始、哪些在推进、哪些已经成单、哪些已经停止，不再依赖口头同步。 | P1 |
| 商机管理 | 商机详情弹窗 | 查看需求信息 | 在商机详情中集中展示客户需求背景、需求联系人、创建方式、创建人、金额和完整需求描述；如果商机来自自动同步，再补充展示来源入口，帮助后续跟进、方案沟通和交付承接保持信息完整一致。 | P1 |
...
| 订单管理 | 创建订单 | 添加设备 / AI 专家 / 专家团 / 积分 | 在同一张订单里清晰录入本次售卖的设备、单个 AI 专家、AI 专家团和积分资源包，形成完整的售卖清单，为后续交付、资产生成和续费提供依据。对于 AI 专家商品，广场卡片默认展示最新版本，但允许在加入订单前切换到指定交付版本。 | P0 |
| 订单管理 | 订单详情 | 查看订单类型 | 在订单详情中明确区分新购订单和续费订单，帮助 FDE 成员快速判断本单是首期交付、增量配置还是续期处理。 | P1 |
| 订单管理 | 订单详情 | 跳转租户管理交付详情 | 订单确认后可直接进入对应交付详情，确保“下单”和“执行交付”之间形成连续链路，减少跨页面反复查找。 | P1 |
...
| 租户管理 | 租户列表 | 工程师 / 负责人视角 | 租户管理首页以租户为单位组织工作对象，负责人和管理员看到全量租户，小组负责人看到本组租户，普通成员只看到自己负责范围，让每个人进入页面就能看到与自己职责相关的交付对象。 | P0 |
| 租户管理 | 租户列表 | 创建租户 | 支持先创建租户，再围绕该租户持续创建订单和推进交付，确保业务主体先被建立，再承接后续多次销售和交付动作。 | P0 |
| 租户管理 | 租户详情 | 查看租户下订单列表 | 进入租户详情后，以订单列表承接该租户下所有待处理和已处理的交付任务，让交付工作按订单逐个推进，而不是混成一个租户级黑盒。 | P0 |
| 租户管理 | 租户详情 | 在租户下新建订单 | 支持在租户上下文中直接建单，减少来回切换，确保新增订单天然绑定到当前租户并能立即进入后续交付流程。 | P0 |
| 租户管理 | 订单交付 | 一订单一交付 | 每个订单都拥有独立交付流程，让新购、增配和续费都能被清晰追踪，避免多次销售动作混在同一条交付记录中。 | P0 |
| 租户管理 | 订单交付 | 按订单预填交付内容 | 订单中的设备和 AI 专家信息会自动带入交付页面，帮助交付人员减少重复录入，但仍保留人工确认和手动推进的控制权。 | P0 |
| 租户管理 | 订单交付 | 设备分配步骤 | 在设备分配阶段完成本次订单设备额度、生效安排和分配确认，确保后续设备资产生成准确，且与订单内容保持一致。 | P0 |
| 租户管理 | 订单交付 | Agent 下发步骤 | 在 Agent 下发阶段完成本次订单对应的专家团或单个 AI 专家下发，让客户租户真正具备可使用的 AI 能力；如需在交付阶段追加 Agent，仍复用专家广场逻辑，默认展示最新版本，并允许切换到指定版本后再加入交付内容。 | P0 |
| 租户管理 | 订单交付 | 企业后台配置 | 在交付过程中引导企业管理员完成后台必要配置，确保交付不仅停留在“资源已发放”，而是真正达到“企业可开始使用”的状态；该步骤常驻提供企业后台入口，并按订单内容直接进入企业管理后台的对应页签。 | P1 |
| 租户管理 | 订单交付 | 交付验收 | 在所有关键配置完成后，由 FDE 对本次交付做最终确认和交接收口，确保租户已具备正式上线使用条件。 | P0 |
| 租户管理 | 订单交付 | 已完成步骤只读 | 对已完成或已跳过的阶段只展示结果，不再提供重复操作入口，避免交付过程中对已确认内容产生误改。 | P1 |
| 客户资产管理 | 租户列表 | 查看已交付租户 | 以租户为单位集中管理已交付后的运行资产，让团队进入资产页时先定位业务主体，再查看其设备、专家和变更情况。 | P1 |
| 客户资产管理 | 资产概况 | 查看额度卡片 | 从租户视角查看当前设备额度、积分额度和积分消耗主数值，帮助 FDE 快速判断客户是否存在扩容需求、使用异常或续费机会。 | P1 |
...
| 客户资产管理 | 资产明细 | 资产变更记录查看 | 把该租户历史上已经实际发生的设备、AI 专家和版本类资产变化统一沉淀下来，方便团队回溯客户为什么会形成当前资产结构。 | P1 |
...
3. 订单商品支持 4 类：
   ...
   - AI 专家团
   - 积分
4. AI 专家商品默认带最新版本，但允许在加入订单前切换到指定交付版本；最终下单版本即为后续交付的执行版本。
5. 设备和 AI 专家商品必须带有效时长，用于后续资产到期与续费。

### 6.4.4 配置交付逻辑

1. 当前原型在导航层以“租户管理”承接该模块，首页按租户展示，而不是按订单直接平铺。
2. 进入租户后，左侧是该租户下的订单列表。
3. 一个订单对应一个交付。
4. 订单只负责预填交付内容，不自动把步骤标完成。
5. Agent 下发阶段若需追加单个 AI 专家或 AI 专家团，复用与订单创建相同的专家广场逻辑，默认展示最新版本，并允许切换到指定版本后再加入本次交付内容。
6. 企业后台配置步骤中的企业后台入口需要常驻显示，避免成员在查看已完成步骤时失去进入后台的路径。
7. 企业后台入口应按订单内容深链到企业管理后台对应页签：含 AI 专家的订单进入「我的AI专家团」，纯设备订单进入「设备管理」。
8. 已完成或已跳过的步骤只读，不再允许重复执行动作。
9. 交付步骤当前包含：
   - 设备分配
   - Agent 下发
   - 企业后台配置
...
6. 资产概况卡片只展示设备额度、积分额度和积分消耗主数值。
7. 资产页中的资产变更记录只承接已经实际发生的资产变化，用于回溯设备、AI 专家和版本是如何逐步演进成当前结果的。
