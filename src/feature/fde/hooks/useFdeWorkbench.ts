...
export const useFdeWorkbench = (): UseFdeWorkbenchResult => {
  ...
  const [releasePushes] = useState<FdeReleasePushItem[]>(FDE_RELEASE_PUSHES);
  ...
  const filteredReleasePushes = useMemo<FdeReleasePushItem[]>(
  ...
  const assignLead = useCallback((leadId: string, memberId: string | null): void => {
    setLeads(previous =>
      previous.map(item => (item.id === leadId ? { ...item, assignedToId: memberId } : item)),
    );
  }, []);

  const updateLeadStatus = useCallback(
    (leadId: string, status: FdeLeadStatus): void => {
      setLeads(previous => previous.map(item => (item.id === leadId ? { ...item, status } : item)));
    ...
    (agentId: string): void => {
      ...
      setEvolutionTasks(previous => {
        ...
        const nextTask: FdeEvolutionTaskItem = {
          ...
          changedSkills: targetAgent.skillTags,
