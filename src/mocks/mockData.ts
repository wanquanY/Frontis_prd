...
const createDepartmentScope = (subjectId: string, subjectName: string): AccessScopeSubject => ({
  subjectId,
  subjectName,
...
export const INITIAL_EMPLOYEES: EmployeeItem[] = [
  ...
  },
  {
    ...
    role: "理解用户目标、处理基础对话并按权限调度 AI 专家",
    portalRoles: ["admin", "employee"],
    status: "idle",
    workspaceId: "workspace-local",
    connectionMode: "cloud",
    model: "gpt-4o",
    summary: "工作台默认入口，负责理解需求、处理文件和在具备权限时调度 AI 专家。",
    lastAction: "等待新的任务输入。",
    source: "coworker",
    visibility: "all",
    developerName: "杨万泉",
    myLaborCost: 30000,
    industryStandardCost: 42000,
    subAgentModel: "gpt-3.5-turbo",
    agentId: "ceo-chat-send-01",
    runtimeAgentId: "rt-chat-send-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: [],
    welcomeMessage: "你直接告诉我目标和上下文，我会先判断怎么处理，再把结果汇总给你。",
    systemPrompt:
  ...
  },
  {
    ...
    name: "架构规划师",
    avatarUrl: getAvatarUrl("employee-architect"),
    role: "梳理系统边界、模块拆分和实施约束",
    portalRoles: ["admin", "employee"],
    status: "online",
    workspaceId: "workspace-cloud",
    connectionMode: "cloud",
    model: "gpt-4o",
    summary: "负责把复杂需求沉到系统架构、模块边界和落地约束，避免方案只停留在页面层。",
    lastAction: "已补齐 Frontis 专家团模块的边界拆分和实施约束。",
    source: "coworker",
    visibility: "all",
    developerName: "陈雪梅",
    myLaborCost: 26000,
    industryStandardCost: 34000,
    subAgentModel: "gpt-4o-mini",
    agentId: "prd-architect-agent-01",
    runtimeAgentId: "rt-architect-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: ["杨万泉", "陈雪梅", "王晨", "李婷", "赵立", "周可"],
    welcomeMessage: "我会先确认边界，再把模块拆分、依赖关系和实现风险一次讲清楚。",
    systemPrompt:
  ...
  },
  {
    id: "employee-growth",
    name: "增长实验官",
    avatarUrl: getAvatarUrl("employee-growth"),
    role: "设计增长实验、转化漏斗和验证方案",
    portalRoles: ["admin", "employee"],
    status: "running",
    workspaceId: "workspace-cloud",
    connectionMode: "cloud",
    model: "gpt-4o",
    summary: "把产品方案转成可验证的增长动作，补齐首屏转化、入口路径和实验节奏。",
    lastAction: "已完成 AI 专家团首屏转化路径和实验假设梳理。",
    source: "coworker",
    visibility: "all",
    developerName: "王晨",
    myLaborCost: 19000,
    industryStandardCost: 25000,
    subAgentModel: "gpt-4o-mini",
    agentId: "prd-growth-agent-01",
    runtimeAgentId: "rt-growth-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: ["杨万泉", "陈雪梅", "王晨", "李婷", "赵立", "周可"],
    welcomeMessage: "你给我目标和现状，我会把它拆成可执行的增长实验和验证指标。",
    systemPrompt:
  ...
  },
  {
    id: "employee-qa",
    name: "交付验收官",
    avatarUrl: getAvatarUrl("employee-qa"),
    role: "校验方案可交付性、验收标准和回归范围",
    portalRoles: ["admin", "employee"],
    status: "online",
    workspaceId: "workspace-cloud",
    connectionMode: "cloud",
    model: "gpt-4-turbo",
    summary: "负责把需求交付链条补齐，提前识别验收标准、上线风险和回归缺口。",
    lastAction: "已整理专家团列表改版的验收清单和回归范围。",
    source: "coworker",
    visibility: "all",
    developerName: "赵立",
    myLaborCost: 17000,
    industryStandardCost: 22000,
    subAgentModel: "gpt-4o-mini",
    agentId: "prd-qa-agent-01",
    runtimeAgentId: "rt-qa-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: ["杨万泉", "陈雪梅", "王晨", "李婷", "赵立", "周可"],
  ...
  },
  {
    id: "employee-data",
    name: "数据洞察师",
    avatarUrl: getAvatarUrl("employee-data"),
    role: "沉淀业务指标、数据面板和异常洞察",
    portalRoles: ["admin", "employee"],
    status: "online",
    workspaceId: "workspace-cloud",
    connectionMode: "cloud",
    model: "gpt-4o",
    summary: "负责把需求背后的关键指标、观察口径和异常信号补齐，避免只看体验不看结果。",
    lastAction: "已更新专家团列表曝光、点击和配置转化指标建议。",
    source: "coworker",
    visibility: "all",
    developerName: "周可",
    myLaborCost: 21000,
    industryStandardCost: 27000,
    subAgentModel: "gpt-4o-mini",
    agentId: "prd-data-agent-01",
    runtimeAgentId: "rt-data-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: ["杨万泉", "陈雪梅", "王晨", "李婷", "赵立", "周可"],
  ...
  },
  {
    ...
    name: "用户访谈官",
    avatarUrl: getAvatarUrl("employee-user-researcher"),
    role: "整理用户反馈、访谈纪要和需求优先级证据",
    portalRoles: ["admin", "employee"],
    status: "online",
    workspaceId: "workspace-cloud",
    connectionMode: "cloud",
    model: "gpt-4-turbo",
    summary: "负责把用户声音转成清晰的问题证据、场景脉络和优先级依据。",
    lastAction: "已补充 AI 专家团入口的用户访谈纪要和核心痛点归纳。",
    source: "coworker",
    visibility: "all",
    developerName: "李婷",
    myLaborCost: 16000,
    industryStandardCost: 21000,
    subAgentModel: "gpt-4o-mini",
    agentId: "prd-user-research-agent-01",
    runtimeAgentId: "rt-user-research-01",
    accessScopeSubjects: [COMPANY_SCOPE],
    boundMembers: ["杨万泉", "陈雪梅", "王晨", "李婷", "赵立", "周可"],
