...
import type { SynClawArtifactItem, SynClawSpaceItem } from "@/feature/synclaw/types";
 ...
 * 提取名称头像文案。
 */
export const getPrdAvatarText = (name: string): string => {
 ...
 * 生成在线随机头像地址。
 */
export const getPrdAvatarUrl = (seed: string): string =>
  `https://api.dicebear.com/9.x/notionists/svg?seed=${encodeURIComponent(seed)}`;

/**
 * PRD 默认 AI 专家头像预设。
 */
export const PRD_EMPLOYEE_AVATAR_PRESETS: string[] = [
  getPrdAvatarUrl("prd-employee-avatar-1"),
  getPrdAvatarUrl("prd-employee-avatar-2"),
  getPrdAvatarUrl("prd-employee-avatar-3"),
  getPrdAvatarUrl("prd-employee-avatar-4"),
  getPrdAvatarUrl("prd-employee-avatar-5"),
  getPrdAvatarUrl("prd-employee-avatar-6"),
];

/**
 * 生成原型页本地 ID。
 */
export const createPrdId = (prefix: string): string =>
  `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

interface PrdWorkspaceActivationInfo {
...
const createPrdActivationCode = (): string =>
 ...
 * 生成边缘工作站激活码信息。
 */
export const createPrdWorkspaceActivationInfo = (): PrdWorkspaceActivationInfo => {
  const activationValidDays = 7;

  return {
    activationCode: createPrdActivationCode(),
 ...
 * 获取边缘工作站激活码标题。
 */
export const getPrdActivationTitle = (activationValidDays?: number): string => {
 ...
 * 获取边缘工作站激活码过期时间提示。
 */
export const getPrdActivationExpireText = (activationExpiresAt?: string): string => {
 ...
 * 解析原型页成果文件可访问地址。
 */
export const resolvePrdArtifactUrl = async (file: SynClawArtifactItem): Promise<string> =>
  file.canonicalPath ?? "";

/**
 * 触发原型页成果文件下载。
 */
export const downloadPrdArtifact = (file: SynClawArtifactItem): void => {
 ...
 * 格式化附件体积展示文案。
 */
export const formatPrdFileSize = (size: number): string => {
  if (size >= 1024 * 1024) {
 ...
 */
export const createPrdComposerAttachment = (file: File): WorkspaceComposerAttachmentItem => ({
  uid: createPrdId("attachment"),
  id: Date.now(),
  name: file.name,
  size: file.size,
 ...
 * 释放附件预览 URL。
 */
export const revokePrdComposerAttachmentPreview = (
 ...
 * 转换为页面消息附件结构。
 */
export const buildPrdAttachmentItem = (
  attachment: WorkspaceComposerAttachmentItem,
): AttachmentItem => ({
  id: attachment.uid,
  name: attachment.name,
  size: attachment.size,
  sizeLabel: formatPrdFileSize(attachment.size),
  mimeType: attachment.mimeType,
  url: attachment.url,
});

const resolvePrdMessageTimestamp = (timeLabel: string): number => {
  if (timeLabel === "刚刚") {
    return Date.now();
  }
...
interface NormalizePrdBlocksOptions {
  actorName: string;
...
const normalizePrdBlocks = (blocks: Block[], options: NormalizePrdBlocksOptions): Block[] =>
  blocks.map(block => {
    ...
    if (block.children?.length) {
      nextBlock.children = normalizePrdBlocks(block.children, {
        ...options,
...
export const buildWorkspaceChatBlocks = (messages: ChatMessage[]): Block[] => {
  ...
  return messages.flatMap(message => {
    const timestamp = resolvePrdMessageTimestamp(message.timeLabel);
    const actorRole = message.role === "user" ? "user" : "assistant";

    if (message.blocks?.length) {
      return normalizePrdBlocks(message.blocks, {
